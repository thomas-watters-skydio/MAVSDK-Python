    ...
    def __eq__(self, to_compare):
        ...
        try:
            ...
            return (
                ...
                and (self.short_description == to_compare.short_description)
                and (self.long_description == to_compare.long_description)
                and (self.unit == to_compare.unit)
                and (self.decimal_places == to_compare.decimal_places)
                and (self.start_value == to_compare.start_value)
                and (self.default_value == to_compare.default_value)
                and (self.min_value == to_compare.min_value)
                and (self.max_value == to_compare.max_value)
            )
...
class FloatParamUpdate:
    ...
    def translate_from_rpc(rpcFloatParamUpdate):
        """ Translates a gRPC struct to the SDK equivalent """
        return FloatParamUpdate(rpcFloatParamUpdate.name, rpcFloatParamUpdate.value)
...
class ComponentInformationServerResult:
    ...
    class Result(Enum):
        ...
        def translate_to_rpc(self):
            if self == ComponentInformationServerResult.Result.UNKNOWN:
                return (
                    component_information_server_pb2.ComponentInformationServerResult.RESULT_UNKNOWN
                )
            if self == ComponentInformationServerResult.Result.SUCCESS:
                return (
                    component_information_server_pb2.ComponentInformationServerResult.RESULT_SUCCESS
                )
            ...
            if self == ComponentInformationServerResult.Result.NO_SYSTEM:
                return (
                    component_information_server_pb2.ComponentInformationServerResult.RESULT_NO_SYSTEM
                )
    ...
    def translate_from_rpc(rpcComponentInformationServerResult):
        ...
        return ComponentInformationServerResult(
            ComponentInformationServerResult.Result.translate_from_rpc(
                rpcComponentInformationServerResult.result
            ),
...
class ComponentInformationServer(AsyncBase):
    ...
    def _setup_stub(self, channel):
        """ Setups the api stub """
        self._stub = component_information_server_pb2_grpc.ComponentInformationServerServiceStub(
            channel
        )

    def _extract_result(self, response):
        """ Returns the response status and description """
        return ComponentInformationServerResult.translate_from_rpc(
            response.component_information_server_result
        )
